...
# Debian/Ubuntu keep the proton modules here; Fedora uses the normal
# site-packages, where they are already importable. Only prepend a path
# that actually exists, so this does not shadow sys.path on other distros.
_DEB = "/usr/lib/python3/dist-packages"
if os.path.isdir(_DEB) and _DEB not in sys.path:
    sys.path.insert(0, _DEB)
